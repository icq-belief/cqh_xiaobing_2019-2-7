...
def get_favor_video(param,max_cursor=None):
    ...
    # url = 'https://www.douyin.com/aweme/v1/aweme/post/?user_id={}&max_cursor=0&count={}'.format(param['user_id'], param['count'])
    headers = {
        'user-agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36',
    ...
    if content:
        for i in content:
            ...
            video = 'https://aweme.snssdk.com/aweme/v1/play/?video_id={}'.format(
                video_uri)  # 原来可以playwm去掉wm,下载无水印视频;现在不行了
            print(video)
            # 下载视频
            time.sleep(10)
            mp4 = requests.get(video, headers=headers, stream=True).content  # 必须加上headers,不然跳转失败!!!
            # open(u'videos/{}-{}.mp4'.format(num, title), 'wb').write(mp4)
            open(u'videos/{}.mp4'.format(num), 'wb').write(mp4)
            num = num + 1
    if max_cursor:
        print(max_cursor)
    if html['has_more'] == 1:
...
if __name__ == '__main__':
    user_id = '79971717305' # 不是抖音ID,是url中的ID(见UserID.png)
